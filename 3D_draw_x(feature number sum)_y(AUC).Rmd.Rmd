...
p4=plot_ly(x=data.plot.s$`Sum of the ranking number`,y=data.plot.s$`AUC`, type="scatter",color=my_palette1,size=10,name="The average of random m features")%>%
layout(xaxis = list(title = 'Sum of feature#'),yaxis = list(title = 'Area Under Curve(AUC)'))
...
library(ggpmisc)
library(ggpp)
my_palette <- c("purple", "blue", "green", "yellow")
size=15
ggplot(data.plot.r, aes(V1, V4,color=2:(length(data.plot[[2]])+1))) +
geom_point(size = 3)+
  scale_color_gradient(colors = my_palette)+
#geom_smooth(formula = y ~ x,method = lm)+
#stat_poly_eq(aes(label = paste(..eq.label.., ..adj.rr.label.., sep = '~~~~')),formula = y ~ x,  parse = TRUE,size = 6,label.x = 0.1,label.y = 0.05)+
#geom_line(aes(y=topn.im.total),linetype="dashed",colour="red",size=1)+
theme_classic()+
